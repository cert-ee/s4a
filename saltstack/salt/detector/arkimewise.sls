{% set path_arkime_wise_ini = salt['cmd.run'](cmd='curl -s http://localhost:4000/api/settings/paths | jq -r .path_arkime_wise_ini', python_shell=True) %}
...
{% set wise_installed = salt['cmd.run'](cmd='curl -s http://localhost:4000/api/components/arkimewise | jq -r .installed', python_shell=True) %}
